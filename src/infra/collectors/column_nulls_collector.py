    ...
    def collect(self, context: CollectionContext) -> MetricResult:
        ...
        for rule in rules:
            ...
            table_rows.append(
                (
                    rule.column_name,
                    format_human_count(null_count),
                    format_percent(null_ratio),
                    rule.scope.value,
                )
            )
        ...
        chart_stats = [
            ...
            for item in sorted(
                chart_stats,
                key=lambda item: (item["null_ratio"], item["null_count"]),
                reverse=True,
            )
        ...
        if chart_stats:
            widgets.append(
                ChartWidget(
                    widget_id="column-profile-null-ratio-chart",
                    title="Top columns by null percentage",
                    subtitle="Columns with the highest percentage of null values.",
                    chart_type=ChartType.BAR,
                    labels=tuple(self._short_label(item["column"]) for item in chart_stats),
                    datasets=(
                        ChartDataset(
                            label="Null %",
                            values=tuple(round(item["null_ratio"] * 100, 2) for item in chart_stats),
                        ),
                    ),
                    height_px=340,
                )
